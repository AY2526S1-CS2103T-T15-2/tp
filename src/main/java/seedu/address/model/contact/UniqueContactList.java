 ...
 * in the UniqueContactList. However, the removal of a contact uses Contact#equals(Object) so
...
public class UniqueContactList implements Iterable<Contact> {
    ...
    /**
     * Returns true if the list contains a contact with the given NRIC.
     */
    public boolean contains(Nric nric) {
        requireNonNull(nric);
        return internalList.stream().anyMatch(contact -> contact.getNric().equals(nric));
    }
